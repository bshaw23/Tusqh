...
**Why this wasn't done now:** it changes the wire contract again -- two lists become one compact item each -- which means `3DTriangulateDualOpt.cs`'s input side (and `3DTriangulateDual.cs`'s, if it's ever revisited) would need to change to match, the same scope of change as the Hexes-flattening fix. Deliberately left as-is per explicit instruction ("let's leave the functionality as is for now") after evaluating the option.
